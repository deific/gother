...
import (
	...
	transaction2 "gother/chapter7/internal/transaction"
	"strconv"
	"strings"
	"testing"
)

func GenerateTransaction(outCash int, inAccount string, toAccount string, prevTxID string, outIdx int) *transaction2.Transaction {
	...
	txIn := transaction2.TxInput{TxID: prevTxIDHash[:], OutIdx: outIdx, PubKey: inAccountHash[:]}
	txOut := transaction2.TxOutput{Value: outCash, HashPubKey: toAccountHash[:]}
	tx := transaction2.Transaction{ID: []byte("This is the Base Transaction!"), Inputs: []transaction2.TxInput{txIn}, Outputs: []transaction2.TxOutput{txOut}}
	tx.SetID()
	return &tx
}
...
func GenerateBlock(txs []*transaction2.Transaction, prevBlock string) *blockchain.Block {
	prevBlockHash := sha256.Sum256([]byte(prevBlock))
	testblock := blockchain.NewBlock(1, prevBlockHash[:], txs)
	return testblock
}
...
func TestSPV(t *testing.T) {
	primeTXs := []*transaction2.Transaction{}
